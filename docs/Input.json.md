...
計算を非同期で実行するかどうか。
非同期（`true`）の場合、リクエストがサーバーに受理された時点でステータスコード202が返る。
ただしこの時点で202が返っても計算が成功することを保証するものではない。

リクエストが受理された場合、そのレスポンスでリクエストを一意に表す `key` が返される。
結果取得方法については別資料に記載する。
...
目的関数地として無視されるが、デポの帰着時間の制限に対しては加味される。
...
最適化計算の制限時間（秒）。
ここで指定された時間だけ最適化計算を行い、その中で得られた最も良い解を出力する。

実際の最適化計算が行われる秒数とここで設定した秒数には誤差がある。
またリクエスト全体にかかる時間の上限を15分としているため、それを超えるような値を入力された場合は短縮される。
実際にかかった時間はレスポンスに記載される。

nullまたはキー自体が無い場合および負の値を入力した場合は自動設定される。
...
経路上で右左折を抑制し直進を優先する。
0の場合は右左折を抑制しない。
1〜10の場合は数値が大きいほど強く抑制する (直進を優先する)。
